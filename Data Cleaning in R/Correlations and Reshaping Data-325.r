...
## 7. Correlation Analysis: Measuring the Strength of Relationships Between Variables ##
 ...
 var_cor <- cor(combined$asian_per, combined$avg_sat_score, use = "pairwise.complete.obs")
round(var_cor,7)
...
apscore_cors <- cor_tib %>% select(variable, high_score_percent) %>% filter(high_score_percent < -0.25 | high_score_percent > 0.25)