...
class_size <- class_size %>% group_by(CSD, `SCHOOL CODE`, `SCHOOL NAME`) %>% summarize(avg_class_size = mean(`AVERAGE CLASS SIZE`),
...
class_size <- class_size %>% mutate(DBN = str_c(CSD,`SCHOOL CODE`, sep=""))%>% mutate(DBN = str_pad(DBN, width=6, side='left',pad="0"))

## 8. Graduation Data: Simplifying the Data Frame ##

graduation <- graduation %>% filter(Cohort == "2006", Demographic == "Total Cohort") %>% select(DBN, `School Name`,`Total Grads - % of cohort`, `Dropped Out - % of cohort`) 
