...
lat <- lat_and_long[,1]
long <- lat_and_long[,2]
hs_directory <- hs_directory %>% mutate(lat = lat, long = long)

## 6. Subsetting strings ##

hs_directory <- hs_directory %>% mutate(lat = str_sub(lat, 2,-1), long = str_sub(long, 1,-2)) %>% mutate_at(vars(lat,long),as.numeric)

## 8. Inner Joins ##

sat_class_size <- sat_results %>% inner_join(class_size, by = "DBN")
ggplot(data = sat_class_size) + aes(x = avg_class_size, y = avg_sat_score) + geom_point()
...
combined <- sat_results %>% full_join(ap_2010, by = "DBN") %>% left_join(class_size, by = "DBN") %>% left_join(demographics, by = "DBN") %>% left_join(graduation, by = "DBN") %>% left_join(hs_directory , by = "DBN")