...
reviews_2 <- reviews_2 %>% select(FILM,RT_user_norm,Metacritic_user_nom,IMDB_norm,Fandango_Ratingvalue)
reviews_2 <- reviews_2 %>% rename(Rotten_Tomatoes=RT_user_norm,Metacritic=Metacritic_user_nom,IMDB=IMDB_norm,Fandango=Fandango_Ratingvalue)

## 2. Understanding Relationships Between Variables ##

library(ggplot2)
ggplot(data = reviews_2, aes(x=Fandango, y=Rotten_Tomatoes))+geom_point()

## 3. Creating Informative Scatter Plots ##

ggplot(data = reviews_2,
   aes(x = Fandango, y = Rotten_Tomatoes)) +
   geom_point(alpha=0.3)+xlim(1,5)+ylim(1,5)+theme(panel.background = element_rect(fill="white"))
