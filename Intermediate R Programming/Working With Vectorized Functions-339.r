...
home_goals_sum <- scores %>% group_by(home_country) %>% summarize(sum(home_goals))

## 7. Summarizing a Data Frame by Multiple Variables ##

away_results <- scores %>% group_by(away_country)%>% summarize(total = sum(away_goals),
...
brazil_goals <- scores %>% filter(home_country == "Brazil" | away_country == "Brazil") %>% mutate(total_goals = home_goals + away_goals) %>% arrange(desc(total_goals))