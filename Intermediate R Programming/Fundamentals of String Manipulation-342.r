...
scores <- scores %>% mutate(lose_country = if_else(home_country == win_country, away_country, home_country))
...
scores <- scores %>% mutate(match_day = match_day)

## 8. Creating New Variables ##

scores <- scores %>% mutate(win_goals = if_else(home_country==win_country, home_goals, away_goals), lose_goals=if_else(home_country==lose_country, home_goals, away_goals))
